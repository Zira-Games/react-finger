import { useMemo, useLayoutEffect, forwardRef, createElement, createContext, memo, useContext, Fragment } from 'react';
...
function useFingerEvents(events) {
    var eventsRef = useMemo(function () { return ({}); }, []);
    useLayoutEffect(function () {
        if (events)
            Object.assign(eventsRef, events);
    });
    return useMemo(function () { return composeFingerEvents(eventsRef); }, []);
}
...
function Finger(type) {
    ...
    var FC = forwardRef(function FingerComponent(props, ref) {
        var _a = splitProps(props), eventProps = _a.eventProps, otherProps = _a.otherProps;
        var events = useFingerEvents(eventProps);
        return createElement(type, __assign(__assign(__assign({}, otherProps), events), { ref: ref }));
    });
...
var FingerProxyContext = createContext(null);
var FingerProxy = memo(function FingerProxy(props) {
    var contextTarget = useContext(FingerProxyContext);
...
var FingerProxyBoundary = memo(function FingerProxyBoundary(props) {
    var children = props.children, others = __rest(props, ["children"]);
    var _a = useMemo(function () { return FingerProxyBoundaryOwner(others); }, []), events = _a[0], target = _a[1];
...
function FingerProxyContainer(type) {
    ...
    var FC = forwardRef(function FingerProxyContainerComponent(props, ref) {
        var eventBoundary = props.eventBoundary, others = __rest(props, ["eventBoundary"]);
        if (eventBoundary === false)
            return createElement(type, __assign(__assign({}, others), { ref: ref }));
        var eventProps = splitProps(others).eventProps;
        return createElement(FingerProxyBoundary, __assign(__assign({}, eventProps), { children: function (events) {
                return createElement(type, __assign(__assign(__assign({}, others), events), { ref: ref }));
            } }));
    });
...
export { Finger, FingerGlobal, FingerOptions, FingerProxy, FingerProxyBoundary, FingerProxyContainer, bindFingerGlobalEvents, composeFingerEvents, createFingerEvents, useFingerEvents };
